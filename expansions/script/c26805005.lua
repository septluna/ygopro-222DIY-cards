...
function c26805005.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(26805005,0))
	e1:SetCategory(CATEGORY_NEGATE)
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DAMAGE_CAL)
	e1:SetCode(EVENT_CHAINING)
	e1:SetRange(LOCATION_HAND)
	e1:SetCountLimit(1,26805005)
	e1:SetCondition(c26805005.discon)
	e1:SetCost(c26805005.discost)
	e1:SetTarget(c26805005.distg)
	e1:SetOperation(c26805005.disop)
	c:RegisterEffect(e1)
end
function c26805005.cfilter(c)
	return c:IsLocation(LOCATION_REMOVED) and c:IsType(TYPE_MONSTER)
end
function c26805005.discon(e,tp,eg,ep,ev,re,r,rp)
	...
	return ((ex1 and (bit.band(dv1,LOCATION_REMOVED)==LOCATION_REMOVED or g1 and g1:IsExists(Card.IsLocation,1,nil,LOCATION_REMOVED)))
		...
		or (ex4 and (bit.band(dv4,LOCATION_REMOVED)==LOCATION_REMOVED or g4 and g4:IsExists(c26805005.cfilter,1,nil)))
		or (ex5 and (bit.band(dv5,LOCATION_REMOVED)==LOCATION_REMOVED or g5 and g5:IsExists(Card.IsLocation,1,nil,LOCATION_REMOVED))))
		and Duel.IsChainNegatable(ev)
end
function c26805005.discost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsDiscardable() end
	Duel.SendtoGrave(c,REASON_COST+REASON_DISCARD)
end
function c26805005.distg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_NEGATE,eg,1,0,0)
end
function c26805005.disop(e,tp,eg,ep,ev,re,r,rp)
