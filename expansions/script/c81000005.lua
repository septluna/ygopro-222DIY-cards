...
function c81000005.initial_effect(c)
	...
	e4:SetCountLimit(1,81000905)
	e4:SetCondition(c81000005.tpcon)
	e4:SetTarget(c81000005.tptg)
	e4:SetOperation(c81000005.tpop)
	c:RegisterEffect(e4)
end
...
function c81000005.tpcon(e,tp,eg,ep,ev,re,r,rp)
...
function c81000005.tpfilter(c,e,tp)
	return c:IsAttribute(ATTRIBUTE_WIND) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c81000005.tptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c81000005.tpfilter,tp,LOCATION_HAND,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND)
end
function c81000005.tpop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c81000005.tpfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp)
