...
function cm.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(61081005,1))
	e2:SetCategory(CATEGORY_TOHAND)
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetRange(LOCATION_FZONE)
	e2:SetCountLimit(1,61081005)
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetFlagEffect(tp,61081005)==0 and Duel.SelectYesNo(tp,aux.Stringid(61081005,2)) then
...
function cm.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.RaiseEvent(tc,EVENT_SSET,e,REASON_COST,tp,tp,0)
end
