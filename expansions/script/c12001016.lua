...
function c12001016.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(12001016,1))
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetRange(LOCATION_GRAVE)
	e2:SetCountLimit(1,12001016)
	e2:SetTarget(c12001016.tdtg)
	e2:SetOperation(c12001016.tdop)
	c:RegisterEffect(e2)
end
...
function c12001016.tdfilter(c,e,tp)
	return c:IsSetCard(0xfb0) and c:IsAbleToDeck()
end
function c12001016.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToDeck()  and Duel.IsExistingMatchingCard(c12001016.tdfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,e:GetHandler()) end
...
function c12001016.tdop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not Duel.IsExistingMatchingCard(c12001016.tdfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,2,e:GetHandler()) then return false end
	if Duel.SendtoDeck(c,nil,0,REASON_EFFECT)~=0 and c:IsLocation(LOCATION_DECK) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
		local g1=Duel.SelectMatchingCard(tp,c12001016.tdfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,99,e:GetHandler())
