...
function c26807001.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(26807001,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetProperty(EFFECT_FLAG_DELAY)
	e1:SetCode(EVENT_TO_HAND)
	e1:SetCountLimit(1,26807001)
	e1:SetCondition(c26807001.spcon)
	e1:SetTarget(c26807001.sptg)
	e1:SetOperation(c26807001.spop)
	c:RegisterEffect(e1)
...
function c26807001.spcon(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsReason(REASON_DRAW)
end
function c26807001.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,0,0)
end
function c26807001.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
