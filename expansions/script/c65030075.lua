...
function c65030075.initial_effect(c)
	...
	e1:SetCondition(c65030075.con)
	e1:SetTarget(c65030075.tg)
	e1:SetOperation(c65030075.op)
	c:RegisterEffect(e1)
	--synchro effect
...
function c65030075.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c65030075.tgfil,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil,e,tp) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 end
	if Duel.GetFlagEffect(tp,65030075)==0 then
		Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND+LOCATION_DECK)
	else
		Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND)
	end
end
function c65030075.op(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	local g=Duel.SelectMatchingCard(tp,c65030075.tgfil,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil,e,tp)
	if g:GetCount()>0 then 
		if g:GetFirst():IsLocation(LOCATION_DECK) then 
			Duel.RegisterFlagEffect(tp,65030075,RESET_PHASE+PHASE_END,0,1)
		end
		Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
	end
end
function c65030075.sctarg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c65030075.scop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
