...
function cm.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(61081003,1))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetRange(LOCATION_SZONE)
	e2:SetCountLimit(1,61081003)
	e2:SetCost(cm.cost)
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetFlagEffect(tp,61081003)==0 and Duel.SelectYesNo(tp,aux.Stringid(61081001,2)) then
		local token=Duel.CreateToken(tp,61080000)
		Duel.SpecialSummon(token,0,tp,tp,false,false,POS_FACEUP)
		Duel.RegisterFlagEffect(tp,61081003,RESET_PHASE+PHASE_END,0,1)
	end
end
