...
function cm.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(61081001,1))
	e2:SetCategory(CATEGORY_SEARCH+CATEGORY_TOHAND)
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetRange(LOCATION_SZONE)
	e2:SetCountLimit(1,61081001)
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetFlagEffect(tp,61081001)==0 and Duel.SelectYesNo(tp,aux.Stringid(61081001,2)) then
		local token=Duel.CreateToken(tp,61080000)
		Duel.SpecialSummon(token,0,tp,tp,false,false,POS_FACEUP)
		Duel.RegisterFlagEffect(tp,61081001,RESET_PHASE+PHASE_END,0,1)
	end
end
