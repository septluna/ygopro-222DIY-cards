--崩坏神格 阿尔忒弥斯
c75646199.dfc_front_side=75646199
c75646199.dfc_back_side=75646265
function c75646199.initial_effect(c)
	...
	e7:SetDescription(aux.Stringid(75646199,1))
	e7:SetType(EFFECT_TYPE_IGNITION)
	e7:SetRange(LOCATION_SZONE)
	e7:SetCost(c75646199.chcost)
	e7:SetTarget(c75646199.changetg)
	e7:SetOperation(c75646199.changeop)
	c:RegisterEffect(e7)
end
...
function c75646199.costfilter(c)
	return c:IsCode(75646266) and c:IsAbleToGraveAsCost()
end
function c75646199.chcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c75646199.costfilter,tp,LOCATION_HAND,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local cg=Duel.SelectMatchingCard(tp,c75646199.costfilter,tp,LOCATION_HAND,0,1,1,nil)
	Duel.SendtoGrave(cg,REASON_COST)
end
function c75646199.changetg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c.dfc_back_side and c.dfc_front_side==c:GetOriginalCode() end
	Duel.Hint(HINT_OPSELECTED,1-tp,e:GetDescription())
end
function c75646199.changeop(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
