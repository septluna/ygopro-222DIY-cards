...
function c12010037.initial_effect(c)
	...
	e1:SetCountLimit(1,12010037+EFFECT_COUNT_CODE_OATH)
...
function c12010037.filter(c,tp)
	return c:IsType(TYPE_TRAP) and (c:IsType(TYPE_CONTINUOUS) or c:IsType(TYPE_COUNTER)) and c:IsControler(tp) and not c:IsPublic()
...
function c12010037.filter1(c,tp)
	return c:IsType(TYPE_SPELL) and c:IsType(TYPE_CONTINUOUS+TYPE_FIELD) and c:IsSetCard(0xfbc) and c:IsControler(tp) and not c:IsPublic()
		and c:IsPreviousLocation(LOCATION_DECK) and c:GetPreviousControler()==tp
end
