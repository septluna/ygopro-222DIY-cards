...
function cm.initial_effect(c)
	...
	e0:SetCountLimit(1,61081008+100)
	--target
	...
	e3:SetDescription(aux.Stringid(61081008,0))
...
function cm.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsFaceup,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,2,nil) end
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,nil,1,0,LOCATION_ONFIELD)
end
function cm.desop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectMatchingCard(tp,Card.IsFaceup,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,2,2,nil)
	local tc=g:GetFirst()
	...
	local tc0=g:GetNext()
	if tc:IsType(TYPE_MONSTER) then
		Duel.SpecialSummon(tc0,0,tp,tp,false,false,POS_FACEDOWN_DEFENSE)
	else
		Duel.ChangePosition(tc0,POS_FACEDOWN)
		Duel.RaiseEvent(tc0,EVENT_SSET,e,REASON_EFFECT,tp,tp,0)
	end
