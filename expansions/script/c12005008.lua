...
function c12005008.sfilter(c,e,tp)
	return c:IsCode(12005012) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0
end
function c12005008.operation(e,tp,eg,ep,ev,re,r,rp)
	local tc=re:GetHandler()
	if not tc:IsDisabled() then
		if Duel.NegateEffect(ev) and Duel.IsExistingMatchingCard(c12005008.filter1,tp,LOCATION_HAND,0,1,nil) then
			local sc=Duel.GetFirstMatchingCard(c12005008.sfilter,tp,LOCATION_EXTRA,0,nil,e,tp)
			if sc and Duel.SelectYesNo(tp,aux.Stringid(12005008,1)) then
